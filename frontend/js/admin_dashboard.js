...
window.onload = async () => {
    ...
    // Load admin name/avatar from profile
    try {
...
function renderNotifications(notifications) {
    ...
    if (notifications && notifications.length > 0) {
        ...
        container.innerHTML = `<p class="text-muted small p-2">No new notifications.</p>`;
    }
}
...
function renderPlacements(placements) {
    ...
    if (placements && placements.length > 0) {
        ...
        container.innerHTML = `<p class="text-muted small p-2">No recent placements found.</p>`;
    }
}
...
async function sendAnnouncement() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            textarea.value = "";
            const modal = bootstrap.Modal.getInstance(document.getElementById('announceModal'));
            if (modal) modal.hide();
            await loadAnnouncements();
        } else {
